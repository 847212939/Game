...
bool CTCPSocketManage::ConnectServer(int& fd)
{
...
void CTCPSocketManage::ConnectServerThread(int& fd)
{
...
bool CTCPSocketManage::SendData(const char* pData, size_t size, int mainID, int assistID, int handleCode, void* pBufferevent, unsigned int uIdentification/* = 0*/)
{
	...
	if (m_pSendDataLine)
	{
		...
		if (addBytes == 0)
		{
			std::cout << "Ͷ����Ϣʧ��,assistID" << assistID << std::endl;
			return false;
		}
	}
